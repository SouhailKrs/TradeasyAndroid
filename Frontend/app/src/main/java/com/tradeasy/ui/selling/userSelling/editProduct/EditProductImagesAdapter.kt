...
// create a list of images
        class EditProductImagesAdapter(val images: MutableList<String>) : RecyclerView.Adapter<EditProductImagesAdapter.MyViewHolder>() {
    ...
    override fun getItemCount(): Int {
     ...
     class MyViewHolder(val binding : ProductImageItemBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(image: String) {
          Glide.with(binding.root.context).load(image).into(binding.productImage)
            binding.additionalInfoCardView.layoutParams.height = (getScreenSize(binding.root.context).first*0.15).toInt()
            binding.productImage.layoutParams.height = (getScreenSize(binding.root.context).first*0.14).toInt()
            binding.additionalInfoCardView.layoutParams.width = (getScreenSize(binding.root.context).second * 0.30).toInt()
