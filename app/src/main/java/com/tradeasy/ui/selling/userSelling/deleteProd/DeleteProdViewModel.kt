package com.tradeasy.ui.selling.userSelling.deleteProd
        ...
        viewModelScope.launch {
            deleteProdUseCase.invoke(req ).onStart {
                ...
                when (baseResult) {
                    is BaseResult.Error -> {
                        state.value =
                            DeleteProdState.ErrorDelete(baseResult.rawResponse)

                    }
                    is BaseResult.Success -> state.value =
                        DeleteProdState.SuccessDelete("success")
                }
            }
        }
    }
...
sealed class DeleteProdState {
    ...
    data class SuccessDelete(val message: String) : DeleteProdState()
    data class ErrorDelete(val rawResponse: WrappedResponse<String>) : DeleteProdState()
}